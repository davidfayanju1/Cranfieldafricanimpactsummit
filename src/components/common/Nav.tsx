...
const Nav = () => {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            {/* Hamburger Menu Button - This transforms into the X when open */}
            <button
      ...
      {/* Mobile Menu Overlay - REMOVED THE DUPLICATE CLOSE BUTTON */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-white z-[999] md:hidden block"
          >
            <div className="flex flex-col h-full pt-24 px-6 overflow-y-auto">
                  ...
                  >
                    {item.href ? (
                      <NavLink
                        ...
                        className="text-2xl font-light tracking-widest uppercase text-gray-900 block py-2"
                      >
                        {item.label}
                      </NavLink>
                    ) : (
                      <div>
                        <p className="text-[10px] tracking-[0.4em] font-black text-emerald-600 uppercase mb-4">
                          {item.label}
                        </p>
                        <div className="space-y-4 pl-2">
                          {item.children?.map((child) => (
                            <NavLink
                              ...
                              to={child.href}
                              onClick={() => setIsOpen(false)}
                              className="text-xl font-light tracking-widest uppercase text-gray-700 block py-2"
                            >
                              {child.label}
                            </NavLink>
                          ))}
              ...
              <div className="py-6">
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={() => setIsOpen(false)}
                  className="block w-full py-4 bg-[#11766E] text-white text-center text-[12px] tracking-widest uppercase font-bold rounded-xl"
                >
                  Join Our Community
                </a>
              </div>
